...
#         #ct=x[:,0,:,:,:]
#         #pet=x[:,1,:,:,:]
#         #x_ct = self.patch_embedding(ct)
#         #x_pet = self.patch_embedding(pet)
#         #(B, N, d) --- ct
#         #(b,N,d) --- PET
#         #x = torch.cat((x_ct, x_pet), axis=1)
...
class ViT(nn.Module):
    ...
    def forward(self, x):
        ...
        if hasattr(self, "cls_token"):
            cls_token = self.cls_token.expand(x_ct.shape[0], -1, -1)
            x_ct = torch.cat((cls_token, x_ct), dim=1)

        hidden_states_out = [] 
        for blk in self.blocks:
            x_ct = blk(x_ct)
            hidden_states_out.append(x_ct)

        x_ct = self.norm(x_ct)

        if hasattr(self, "classification_head"):
            x_ct = self.classification_head(x_ct[:, 0])
                
        return x_ct, hidden_states_out



########################################################################################
    # def forward(self, x): # 2 PATCH EMBEDDINGS FOR ct and PET 
    #     ct = x[:, 0, :, :, :].unsqueeze(1)  # Extract CT scan
    ...
    #     if hasattr(self, "cls_token"):
    #         cls_token = self.cls_token.expand(x.shape[0], -1, -1)
    #         x_ct = torch.cat((cls_token, x), dim=1)
